...
- **Which skills** to load — a small curated keyword table (e.g. a charting task pulls in `dataviz`), *plus* `core/skills_discovery.py` scans every skill actually installed on this machine (`~/.claude/skills/`, `~/.claude/plugins/`, and the target project's own `.claude/skills/`) and matches its description against the mission brief for code-writing roles (`BRANCH`, `ROOK`, `BOOTHROYD`, `REVIEW`) — so a newly-installed skill (a Stitch plugin, a design system, a niche framework helper) is usable without a code change to teach Cressida about it. Every code-writing task also always gets **`ponytail`** — anti-over-engineering guidance (YAGNI, stdlib-first, simplest-thing-that-works) — regardless of whether the brief happens to say "simplify."
